...
namespace PizzaHAL
{
    internal class Pizzas: Item
    {
        ...
        private const String Pepperoni = "Pepperoni";
        ...
        public static (String, List<String>) PrintPizzas()
        {
            String ChosenSize = "";
            Console.WriteLine("We make our pizzas with the highest quality meat-based cheese available." + Environment.NewLine + "Please pick which size cheese pizza you'd like. A Small is 10 inches, a Medium is 12 inches, " + Environment.NewLine+
                "a Large is 14 inches, and an Extra-Large is 16 inches.");
            String input = Console.ReadLine();
            while (!string.Equals(input, Small, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Medium, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Large, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, XLarge, StringComparison.OrdinalIgnoreCase))
            {
            ...
            Console.WriteLine("What toppings would you like to add? Please say \"Done\" when you are finished.");
            ...
            {
                while (!string.Equals(input, Pepperoni, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Pineapple, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Bacon, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Mushrooms, StringComparison.OrdinalIgnoreCase)
                    && !string.Equals(input, Sausage, StringComparison.OrdinalIgnoreCase) && !string.Equals(input, Peppers, StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine("Invalid input. Please select a topping or say \"Done\" if you are done.");
                    input = Console.ReadLine();
                }
                ChosenToppings.Add(input);
                Console.WriteLine("Add more toppings? Enter your next topping or say \"Done\" to be done.");
                input = Console.ReadLine();
            }
